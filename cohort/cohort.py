...
class CohortProcessor():
    ...
    def load_database(self):
        ...
        #print ("start times: ", self.start_times_military)
    ...
    #
    def agency_hackathon_load_centroids_npy(self, files_dir):

        # search for each file loaded in the directory
        self.feature_tracks_npy = []
        for k in range(len(self.fnames_slp)):
            fname = os.path.join(files_dir,
                                self.fnames_slp[k][0][:-4]
                                +"_"
                                +self.NN_type[k][0]
                                +'_spine_nohuddle.npy')
            if os.path.exists(fname):
                if self.verbose:
                    print ("file exists...loading ", fname)
                self.feature_tracks_npy.append(np.load(fname))

    #
    def agency_hackathon_load_npy(self, files_dir):
